...
void audio_system::Init()
{
    ...
    Channels[AudioChannel_Music] = SDL_OpenAudioDevice(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, NULL);
    Channels[AudioChannel_SFX]   = SDL_OpenAudioDevice(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, NULL);
...
void audio_system::PauseAll()
{
    ...
    SDL_PauseAudioDevice(Channels[AudioChannel_Music]);
    SDL_PauseAudioDevice(Channels[AudioChannel_SFX]);
}
...
void audio_system::ResumeAll()
{
    ...
    SDL_ResumeAudioDevice(Channels[AudioChannel_Music]);
    SDL_ResumeAudioDevice(Channels[AudioChannel_SFX]);
}
...
void audio_system::SetGlobalVolume(f32 Volume)
{
    ...
    SDL_SetAudioDeviceGain(Channels[AudioChannel_Music], Volume);
    SDL_SetAudioDeviceGain(Channels[AudioChannel_SFX], Volume);
}
...
sound audio_system::CreateSound(const std::string &Path, audio_channel Channel, bool Repeats)
{
    ...
    Log(Info, "audio_system::CreateSound()  - Creating sound %s, bound to Channel: %s", Path.c_str(), Channel == AudioChannel_Music ? "AudioChannel_Music" : "AudioChannel_SFX");
...
{
    if(Sound.Stream == nullptr || Sound.Buffer == nullptr || Sound.Length == 0 || Sound.Channel == AudioChannel_Invalid)
    {
    ...
    Sound.Buffer = 0;
    Sound.Length = 0;
    Sound.Channel = AudioChannel_Invalid;
}
